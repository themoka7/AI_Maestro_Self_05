...
분류는 **선택 단계**입니다. `ANTHROPIC_API_KEY` 가 없으면 건너뛰고 복제율 지표만
산출합니다. 키 하나 때문에 이미 수집한 기사와 이미 지불한 검색 API 호출을 버리지
않기 위함입니다. 이때 자율성 점수는 계산하지 않고 비워 둡니다 — 감시 비율 0 은
"취재를 안 했다"가 아니라 "재지 못했다"이므로 0 으로 채워 순위를 매기면 거짓말이 됩니다.

대시보드의 지평선은 그 경우 **100 − 복제율** 로 거리를 잡습니다. 보도자료를 그대로
옮길수록 기준선에 붙는다는 뜻이라 은유가 그대로 성립하고, 복제율은 순수 텍스트
비교라 키 없이도 항상 계산됩니다.
